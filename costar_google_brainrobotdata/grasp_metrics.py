...
def encode_xyz_qxyzw_to_xyz_aaxyz_nsc(xyz_qxyzw, rescale_meters=4):
...
def batch_encode_xyz_qxyzw_to_xyz_aaxyz_nsc(batch_xyz_qxyzw, rescale_meters=4):
    """ Expects n by 7 batch with xyz_qxyzw
    """
    encoded_poses = []
    for xyz_qxyzw in batch_xyz_qxyzw:
        # print('xyz_qxyzw: ' + str(xyz_qxyzw))
        xyz_aaxyz_nsc = encode_xyz_qxyzw_to_xyz_aaxyz_nsc(xyz_qxyzw)
...
def grasp_accuracy_xyz_aaxyz_nsc(y_true_xyz_aaxyz_nsc, y_pred_xyz_aaxyz_nsc, max_translation=0.01, max_rotation=0.261799):
    """ Expects batch of data as an nx8 array.
    ...
    for y_true, y_pred in zip(y_true_xyz_aaxyz_nsc, y_pred_xyz_aaxyz_nsc):
        ...
        translation = tf.norm(y_true_xyz_qxyzw[:3] - y_pred_xyz_qxyzw[:3])
        y_true_q = Quaternion(y_true_xyz_qxyzw[3:])
        y_pred_q = Quaternion(y_true_xyz_qxyzw[3:])
        if translation < max_translation and Quaternion.absolute_distance(q0, q1) < max_rotation:
            accuracies = accuracies + [1]
        else:
            accuracies = accuracies + [0]

    return tf.mean(accuracies)